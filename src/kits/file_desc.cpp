...
w_rc_t file_desc_t::find_fid(ss_m* db)
{
    ...
    w_keystr_t kstr;
    kstr.construct_regularkey(_name, strlen(_name));
    W_DO(ss_m::find_assoc(root_iid(), kstr, &info, infosize, found));
    _fid = info.fid();
      ...
      _fid(fid)
{
    ...
    strncpy(_fname, str.c_str(), str.length());
}
